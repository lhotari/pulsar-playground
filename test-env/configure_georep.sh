...
#pulsar-admin clusters create --tls-enable --tls-allow-insecure --tls-trust-certs-filepath /pulsar/certs/tls.crt --auth-plugin org.apache.pulsar.client.impl.auth.AuthenticationToken --auth-parameters file:///pulsar/token-superuser-stripped.jwt --broker-url pulsar://${peer_cluster_dns}:6650 --broker-url-secure pulsar+ssl://${peer_cluster_dns}:6651 --url http://${peer_cluster_dns}:8080 --url-secure https://${peer_cluster_dns}:8443 ${peer_cluster_name}
